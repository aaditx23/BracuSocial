...
exports.getProcessedSchedule = async (email, password, sessionCode) => {
  try {
    const scheduleWithLab = await usis.classAndLabSchedule(
      email,
      password,
      sessionCode
    );
    ...
    const scheduleRoom = await usis.classOnlySchedule(
      email,
      password,
      sessionCode
    );
    ...
    const finalJson = await processScheduleWithRoom(
      processedScheduleWithLab,
      classrooms
    );
