...
async function scrapeScheduleUrl(baseUrl) {
  try {
    ...
    const pdfLink = $(
      'strong:contains("Class schedule") a[href*="www.bracu.ac.bd/sites/default/files/"]'
    ).attr("href");
...
exports.getProcessedSchedule = async (currentSemester, currentYear) => {
  try {
    ...
    const scraped = await scrapeScheduleUrl(
      `https://www.bracu.ac.bd/class-schedule-${currentSemester}-${currentYear}`
    );
    if (!scraped.pdfUrl) {
