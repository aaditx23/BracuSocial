...
const CourseList: React.FC<CourseListProps> = ({
    ...
    const fetchCourses = async () => {
      ...
      try {
        const response = await axios.get<Course[]>(
          "https://bracusocial-web-backend.vercel.app/api/pdf/schedules"
        );
  ...
  const filterCourses = () => {
    const filtered = courses.filter((courseItem) => {
      return (
        (course === "" ||
          courseItem.course.toLowerCase().includes(course.toLowerCase())) &&
        (section === "" || courseItem.section.includes(section)) &&
        (faculty === "" ||
          courseItem.faculty.toLowerCase().includes(faculty.toLowerCase())) &&
        (room === "" ||
                    ...
                    {filteredCourses.map((courseItem) => (
                      <TableRow
                        ...
                        className={`cursor-pointer transition-colors ${
                          addedCourses.some(
                            (addedCourse) =>
                              addedCourse.course === courseItem.course
                          )
